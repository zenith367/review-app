...
export default function AddReview() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMovieTitle(""); setMovieId(""); setComment(""); setRating(5);
      navigate("/my-reviews");
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="container mt-4">
      ...
      <form onSubmit={handleSubmit}>
        <input className="form-control mb-3" placeholder="Movie Title" value={movieTitle} onChange={(e) => setMovieTitle(e.target.value)} required />
        <input className="form-control mb-3" placeholder="Movie ID (optional)" value={movieId} onChange={(e) => setMovieId(e.target.value)} />
        <textarea className="form-control mb-3" placeholder="Write your review..." value={comment} onChange={(e) => setComment(e.target.value)} required />
        <input className="form-control mb-3" type="number" min="1" max="5" value={rating} onChange={(e) => setRating(e.target.value)} required />
