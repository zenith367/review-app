...
export default function MyReviews() {
  ...
  const fetchMyReviews = async () => {
    try {
      setLoading(true);
      const res = await api.get("/api/reviews"); // updated route
      const all = res.data || [];
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      await api.delete(`/api/reviews/${id}`, { headers }); // updated route
      ...
      {reviews.map((r) => (
        <div key={r.id} className="card mb-3 p-3 shadow-sm">
          <div className="d-flex justify-content-between align-items-start">
            ...
            <div>
              <button className="btn btn-sm btn-outline-primary me-2" onClick={() => startEdit(r)}>
                Edit
              </button>
          ...
          {editingId === r.id && (
            <div className="mt-3">
              ...
              />
              <div>
                <button className="btn btn-success me-2" onClick={() => submitEdit(r.id)}>
                  Save
                </button>
